...
def fractal_terrain(terrain, levels=8, scale=1.0):
    ...
    terrain.height_field = height.astype(np.int16)
    return terrain
...
def sloped_terrain(terrain, slope=1):
    ...
    terrain.height_field[:, np.arange(terrain.length)] += (max_height * xx / terrain.width).astype(
        terrain.height_field.dtype
    )
    return terrain
...
def discrete_obstacles_terrain(terrain, max_height, min_size, max_size, num_rects, platform_size=1.0):
    ...
    for _ in range(num_rects):
        ...
        terrain.height_field[start_i : start_i + width, start_j : start_j + length] = np.random.choice(height_range)
...
def wave_terrain(terrain, num_waves=1, amplitude=1.0):
    ...
    if num_waves > 0:
        ...
        xx = xx.reshape(terrain.width, 1)
        yy = yy.reshape(1, terrain.length)
        terrain.height_field += (amplitude * np.cos(yy / div) + amplitude * np.sin(xx / div)).astype(
            terrain.height_field.dtype
        )
    return terrain
...
def stepping_stones_terrain(terrain, stone_size, stone_distance, max_height, platform_size=1.0, depth=-10):
    ...
    start_x = 0
    start_y = 0
    terrain.height_field[:, :] = int(depth / terrain.vertical_scale)
    if terrain.length >= terrain.width:
        while start_y < terrain.length:
            ...
            terrain.height_field[0:stop_x, start_y:stop_y] = np.random.choice(height_range)
            # fill row
            while start_x < terrain.width:
                stop_x = min(terrain.width, start_x + stone_size)
                terrain.height_field[start_x:stop_x, start_y:stop_y] = np.random.choice(height_range)
        ...
        while start_x < terrain.width:
            ...
            terrain.height_field[start_x:stop_x, 0:stop_y] = np.random.choice(height_range)
            # fill column
            while start_y < terrain.length:
                stop_y = min(terrain.length, start_y + stone_size)
                terrain.height_field[start_x:stop_x, start_y:stop_y] = np.random.choice(height_range)
...
def convert_heightfield_to_trimesh(height_field, horizontal_scale, vertical_scale, slope_threshold=None):
    """
    ...
    Parameters:
        height_field (np.array): input heightfield
